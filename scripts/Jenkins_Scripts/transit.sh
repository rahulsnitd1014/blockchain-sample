...
cd $BASE_FOLDER/trace-blockchain-sample/transit-fabric || exit
export PATH=gopath/src/github.com/hyperledger/fabric-samples/bin:$PATH

LANGUAGES="go javascript typescript"
for LANGUAGE in ${LANGUAGES}; do
    echo -e "\033[32m starting transit-fabric test (${LANGUAGE})" "\033[0m"
    # Start Fabric, and deploy the smart contract
    ./startFabric.sh ${LANGUAGE}
    copy_logs $? transit-fabric-${LANGUAGE}
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        copy_logs $? transit-fabric-${LANGUAGE}-enrollAdmin
        ${COMMAND} ${PREFIX}registerUser${SUFFIX}
        copy_logs $? transit-fabric-${LANGUAGE}-registerUser
        ${COMMAND} ${PREFIX}query${SUFFIX}
        copy_logs $? transit-fabric-${LANGUAGE}-query
        ${COMMAND} ${PREFIX}invoke${SUFFIX}
        copy_logs $? transit-fabric-${LANGUAGE}-invoke
        popd
    fi
    docker ps -aq | xargs docker rm -f
    docker rmi -f $(docker images -aq dev-*)
    echo -e "\033[32m finished transit-fabric test (${LANGUAGE})" "\033[0m"
done
